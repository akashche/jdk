...
/*
 * @test
 * @summary test installation of the jdk installer with only "jdk" feature selected
 * @library ../../tools/jpackage/helpers
 * @library ../helpers
 * @requires (os.family == "windows")
 * @modules jdk.jpackage/jdk.jpackage.internal
 *
 * @compile FilesOnlyTest.java
 * @run main/othervm jdk.jpackage.test.Main --jpt-run=FilesOnlyTest
 */
public class FilesOnlyTest {
    ...
    public static void test() {
        ...
        if (memberOfAdministratorsGroup()) {
            ...
            // install MSI and check installed files
            Path installed = install(msi, "ADDLOCAL=jdk");
            try {

                assertInstalledFiles(image, installed);
                assertNoRegistryRuntime(installed);
                assertNoEnvPath(installed);
