...
public class ExtendJaxbTest {
    ...
    private static void transformDescriptor(Path instBuildDir) throws Exception {
        ...
        Path extendJaxbJava = srcRoot.resolve("test/jdk/installermsi/extend/ExtendJaxb.java");
        ...
        if (0 != status) {
            throw new RuntimeException("Error transforming descriptor");
        }
    }

    private static String collectClasspath() throws Exception {
        Path libs = findJaxbLibsDir();
        StringBuilder sb = new StringBuilder();
        for (String child : libs.toFile().list()) {
            if (child.endsWith(".jar")) {
                sb.append(libs.resolve(child).toAbsolutePath().toString());
                sb.append(";");
            }
        }
        return sb.toString();
