...
/*
 * @test
 * @summary test JavaScript extension example for jdk installer
 * @library ../../tools/jpackage/helpers
 * @library ../helpers
 * @requires (os.family == "windows")
 * @modules jdk.jpackage/jdk.jpackage.internal
 *
 * @compile ExtendScriptTest.java
 * @run main/othervm jdk.jpackage.test.Main --jpt-run=ExtendScriptTest
 */
public class ExtendScriptTest {

    @Test
    public static void test() throws Exception {
        // skip test if Deno runtime not available
        if(null == queryRegistryValue(SYSTEM_ENVIRONMENT_REGKEY, DENO_HOME)) {
            System.err.println("WARNING: Deno runtime not available, please check" +
                    " '" + DENO_HOME + "' system environment variable," +
                    " test run skipped.");
            return;
        }
    ...
    private static Path transformXml(Path instBuildDir) {
        ...
        Path extendScript = testSrcDir.resolve("extended/example/ExtendScript.js");
